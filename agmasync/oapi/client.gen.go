...
type ClientInterface interface {
	...
	// It also carries `RESET_MASTERDATA_SYNC` when the user has wiped a tenant's master data: every canonical object, identifier mapping, masterdata route and initial-load state of the tenant is gone, and the application MUST discard its bindings there. It reaches every application holding a binding in the tenant, including one with no endpoint left there, so it may concern a tenant the application is no longer routed to. It precedes any later event of that tenant, catch-up included. The application MUST durably store a position at or past it before taking part in that tenant's initial load again.
	//
